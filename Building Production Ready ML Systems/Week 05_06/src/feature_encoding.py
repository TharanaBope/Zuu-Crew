...
# creating abstract class for feature encoding strategies
class FeatureEncodingStrategy(ABC):
    @abstractmethod
    def encode(self, df: pd.DataFrame) ->pd.DataFrame:
        pass

# creating enum for variable types
class VariableType(str, Enum):
    NOMINAL = 'nominal'
    ORDINAL = 'ordinal'

# creating nominal encoding strategy
class NominalEncodingStrategy(FeatureEncodingStrategy):
    # Constructor for NominalEncodingStrategy
    def __init__(self, nominal_columns):
        self.nominal_columns = nominal_columns
        self.encoder_dicts = {}
        os.makedirs('artifacts/encode', exist_ok=True)

    # Encode the DataFrame using nominal encoding
    def encode(self,df):
        for column in self.nominal_columns:
            ...
            encoder_path = os.path.join(
                'artifacts/encode', f"{column}_encoder. json"
                )
    ...
    # Get the encoder dictionaries
    def get_encoder_dicts(self):
        return self.encoder_dicts

# creating ordinal encoding strategy
